...
const DEFAULT_AMAP_SECURITY_CODE = "";
...
export function getStoredAmapConfig() {
  return {
    ...
    securityCode:
      localStorage.getItem("amap_security_code") || DEFAULT_AMAP_SECURITY_CODE,
  };
}
...
export async function loadAMap(): Promise<any> {
  ...
  try {
    ...
    // ElMessage.error('高德地图加载失败，请检查 API Key 和网络连接')
    if (error === "请填写key") {
      ElMessage.warning(
        "高德地图 API Key 为空，请前往设置（右上角齿轮）填写有效的 Key"
      );
    }
    throw error;
  }
...
export function formatAddress(fullAddress: string): string {
  // 移除省份和城市前缀，保留区县及后续内容
  const patterns = [/^.*?(市|自治州|地区|盟|县)(.*)/, /^.*?(区|县)(.*)/];
