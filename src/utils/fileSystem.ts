...
export async function requestDirectoryAccess(
  ...
  if (!isFileSystemAccessSupported()) {
    throw new Error(
      "当前浏览器不支持 File System Access API，请使用 Chrome 86+ 或 Edge 86+"
    );
  }
...
/** 获取已保存的文件夹句柄并验证权限 */
export async function getValidDirectoryHandle(
  id: string = "userPhotosFolder"
...
/** 生成文件的 Blob URL */
export async function createBlobUrlFromHandle(
  handle: FileSystemFileHandle
): Promise<string> {
  ...
  try {
    ...
    for await (const entry of subDir.values()) {
      if (entry.kind === "file") {
        ...
        if (types.includes("image") && imageExtensions.test(name))
          isMatch = true;
        if (types.includes("video") && videoExtensions.test(name))
          isMatch = true;
...
export async function moveToTrash(
  ...
  try {
    ...
    try {
      trashHandle = await dirHandle.getDirectoryHandle(".trash", {
        create: true,
      });
    } catch (e) {
      trashHandle = await dirHandle.getDirectoryHandle("trash_bin", {
        create: true,
      });
    }
    ...
    // 4. 在 .trash 中创建新文件
    const newFileHandle = await trashHandle.getFileHandle(newFileName, {
      create: true,
    });
