  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains("landlords")) {
        const landlordStore = db.createObjectStore("landlords", {
          keyPath: "id",
        });
        landlordStore.createIndex("wechatStatus", "wechatStatus", {
          unique: false,
        });
        landlordStore.createIndex("contactStatus", "contactStatus", {
          unique: false,
        });
        landlordStore.createIndex("landlordType", "landlordType", {
          unique: false,
        });
...
export async function getAllLandlords(): Promise<Landlord[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      const landlords = request.result as Landlord[];
      // 数据迁移：将旧格式的 phoneNumbers (string[]) 转换为新格式 ([string, string][])
      const migratedLandlords = landlords.map((landlord) => {
        if (landlord.phoneNumbers && landlord.phoneNumbers.length > 0) {
          // 检查第一个元素是否为数组，如果不是则需要迁移
          const firstPhone = landlord.phoneNumbers[0];
          if (typeof firstPhone === "string") {
            // 旧格式：string[]，转换为新格式 [string, string][]
            return {
              ...landlord,
              phoneNumbers: (landlord.phoneNumbers as any as string[]).map(
...
export async function findLandlordsByPhone(phone: string): Promise<Landlord[]> {
  ...
  return allLandlords.filter((landlord) =>
    landlord.phoneNumbers.some(([phoneNumber]) => phoneNumber === phone)
  );
}
