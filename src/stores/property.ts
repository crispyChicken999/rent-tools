...
export const usePropertyStore = defineStore('property', () => {
  ...
  const filteredLandlords = computed(() => {
    ...
    if (filters.value.wechatStatus && filters.value.wechatStatus.length > 0) {
      result = result.filter(l => filters.value.wechatStatus!.includes(l.wechatStatus))
    }

    if (filters.value.contactStatus && filters.value.contactStatus.length > 0) {
      result = result.filter(l => filters.value.contactStatus!.includes(l.contactStatus))
    }

    if (filters.value.landlordType && filters.value.landlordType.length > 0) {
      result = result.filter(l => filters.value.landlordType!.includes(l.landlordType))
    }
    ...
    if (filters.value.rentRange) {
      ...
      result = result.filter(l =>
        l.properties.some(p => p.rent >= min && p.rent <= max)
      )
    }
  ...
  const perfectCount = computed(() => landlords.value.filter(l => l.isPerfect).length)
  const imperfectCount = computed(() => landlords.value.filter(l => !l.isPerfect).length)
  const contactedCount = computed(() => landlords.value.filter(l => l.contactStatus === ContactStatus.Contacted).length)
  ...
  async function mergeLandlords(targetId: string, sourceId: string) {
    ...
    // 合并电话号码（去重）
    const mergedPhones = Array.from(new Set([...target.phoneNumbers, ...source.phoneNumbers]))
  ...
  /** 添加房源到房东 */
  async function addProperty(landlordId: string, property: Omit<Property, 'id' | 'landlordId' | 'createdAt' | 'updatedAt'>) {
  ...
  /** 更新房源 */
  async function updateProperty(landlordId: string, propertyId: string, updates: Partial<Property>) {
    const landlord = landlords.value.find(l => l.id === landlordId)
    if (!landlord) {
      throw new Error('房东不存在')
    }
  ...
  async function removeProperty(landlordId: string, propertyId: string) {
    ...
    const updatedProperties = landlord.properties.filter(p => p.id !== propertyId)
  ...
  /** 设置当前查看的房东 */
  function setCurrentLandlord(landlord: Landlord | null) {
